...
module.exports = {
  ...
  .addChannelOption(opt => opt
    .setName('category')
    .setDescription('Change channel category! (choose the same category for just changing position)')
    .addChannelTypes(4)
    .setRequired(true)
  )
    ...
    switch(true) {
     ...
     return interaction.reply({
      ...
      case (channel.parent.id === category.id): {
        ...
        .setDescription('Channel position successfully changed!')
        .addFields({
          name: 'Channels',
          value: `${channel}`,
          inline: true
        ...
        .catch((e) => {
          interaction.reply({
            embeds: [failEmbed.setDescription(`Failed to move ${channel} to position \` ${position} \`\n**Reason:**\n\`\`\`${e}\`\`\``)],
            ephemeral: true
          });
      ...
      case (!position): {
        ...
        .setColor(client.gColor)
        .setTitle('Channel Moved!')
        .setDescription('Channel parent category successfully changed!')
        .addFields({
          name: 'Channels',
          value: `${channel}`,
          inline: true
        ...
        .catch((e) => {
          interaction.reply({
            embeds: [failEmbed.setDescription(`Failed to move ${channel} to ${category}\n**Reason:**\n\`\`\`${e}\`\`\``)],
            ephemeral: true
          });
        return console.log(e)
        });
      ...
      default: {
        ...
        .setColor(client.gColor)
        .setTitle('Channel Moved!')
        .setDescription('Channel position and parent category have successfully changed!')
        .addFields({
          name: 'Channels',
          value: `${channel}`,
        },
        ...
        .catch((e) => {
          interaction.reply({
            embeds: [failEmbed.setDescription(`Failed to move ${channel} to ${category} with \` ${position} \` position.\n**Reason:**\n\`\`\`${e}\`\`\``)],
            ephemeral: true
          });
        return console.log(e)
        });
